...
class AttentionPatternTester:
    ...
    def create_content_matching_task(self, num_samples: int = 1000, seq_len: int = 16) -> Tuple[torch.Tensor, torch.Tensor]:
        """Content-based matching: Find the token that appears twice"""
        inputs = []
        targets = []
        
        for _ in range(num_samples):
            # Create sequence with one repeated token
            seq = torch.randint(5, min(25, self.vocab_size), (seq_len - 1,))
            
            # Pick a random position to repeat
            repeat_pos = np.random.randint(0, seq_len // 2)
            repeat_token = seq[repeat_pos]
            
            # Make sure this token appears exactly twice
            for i in range(len(seq)):
                if i != repeat_pos and seq[i] == repeat_token:
                    seq[i] = (repeat_token.item() + 1) % (self.vocab_size - 5) + 5
            
            # Place the repeated token at a random later position
            second_pos = np.random.randint(seq_len // 2, seq_len - 1)
            seq[second_pos] = repeat_token
            
            # Add query token at end
            input_seq = torch.cat([seq, torch.tensor([self.QUERY_TOKEN])])
            
            # Target: should output the repeated token
            target = torch.zeros(seq_len, dtype=torch.long)
            target[-1] = repeat_token
    ...
    def create_inhibition_task(self, num_samples: int = 1000, seq_len: int = 16) -> Tuple[torch.Tensor, torch.Tensor]:
        """Inhibition: Copy the token that appears only once (ignore repeated tokens)"""
        inputs = []
        targets = []
        ...
        for _ in range(num_samples):
            ...
            # Make most tokens appear twice, but keep one unique
            unique_pos = np.random.randint(0, seq_len - 1)
            unique_token = seq[unique_pos]
            
            # Make 2-3 other tokens appear multiple times
            for _ in range(2):
                if np.random.random() < 0.7:
                    distractor_pos = np.random.randint(0, seq_len - 1)
                    if distractor_pos != unique_pos:
                        distractor_token = seq[distractor_pos]
                        # Find another position for this token
                        other_pos = np.random.randint(0, seq_len - 1)
                        if other_pos != unique_pos and other_pos != distractor_pos:
                            seq[other_pos] = distractor_token
            
            # Ensure unique token appears only once
            for i in range(len(seq)):
                if i != unique_pos and seq[i] == unique_token:
                    seq[i] = (unique_token.item() + 2) % (self.vocab_size - 5) + 5
            
            # Add query
            input_seq = torch.cat([seq, torch.tensor([self.QUERY_TOKEN])])
            
            # Target: the unique token
            target = torch.zeros(seq_len, dtype=torch.long)
            target[-1] = unique_token
    ...
    def create_multistep_task(self, num_samples: int = 1000, seq_len: int = 20) -> Tuple[torch.Tensor, torch.Tensor]:
        """Multi-step reasoning: [A B] [C D] [A] -> [B], but only if C != B"""
        inputs = []
        targets = []
        
        for _ in range(num_samples):
            # Create two patterns: A->B and C->D
            A = torch.randint(5, min(20, self.vocab_size), (1,))
            B = torch.randint(5, min(20, self.vocab_size), (1,))
            C = torch.randint(5, min(20, self.vocab_size), (1,))
            D = torch.randint(5, min(20, self.vocab_size), (1,))
            
            # Ensure all different
            while B == A: B = torch.randint(5, min(20, self.vocab_size), (1,))
            while C == A or C == B: C = torch.randint(5, min(20, self.vocab_size), (1,))
            while D == A or D == B or D == C: D = torch.randint(5, min(20, self.vocab_size), (1,))
            
            # Random middle content
            middle_len = np.random.randint(3, seq_len - 8)
            middle = torch.randint(5, min(20, self.vocab_size), (middle_len,))
            
            # Input: [A B] [middle] [C D] [middle] [A] [padding]
            input_seq = torch.cat([
                A, B, middle[:middle_len//2], C, D, middle[middle_len//2:], A,
                torch.zeros(seq_len - middle_len - 6, dtype=torch.long)
            ])
            
            # Target: predict B after the final A, but only if C != B (add complexity)
            target_seq = torch.zeros(seq_len, dtype=torch.long)
            if C != B:  # Only if no interference
                target_seq[middle_len + 6] = B  # Position after final A
    ...
    def create_interference_task(self, num_samples: int = 1000, seq_len: int = 18) -> Tuple[torch.Tensor, torch.Tensor]:
        """Interference: [A B] [A C] [A] -> [B] (first pattern wins)"""
        inputs = []
        targets = []
        ...
        for _ in range(num_samples):
            ...
            # Random separator
            middle_len = np.random.randint(2, seq_len - 8)
            middle = torch.randint(5, min(20, self.vocab_size), (middle_len,))
            
            # Input: [A B] [middle] [A C] [middle2] [A] [padding]
            middle2_len = max(1, seq_len - middle_len - 7)
            middle2 = torch.randint(5, min(20, self.vocab_size), (middle2_len,))
            
            input_seq = torch.cat([
                A, B, middle, A, C, middle2, A,
                torch.zeros(max(0, seq_len - middle_len - middle2_len - 6), dtype=torch.long)
            ])
            
            # Target: should predict B (first pattern), not C (interference)
            target_seq = torch.zeros(seq_len, dtype=torch.long)
            final_a_pos = middle_len + middle2_len + 5
            if final_a_pos < seq_len:
                target_seq[final_a_pos + 1] = B  # Position after final A
    ...
    def create_variable_length_task(self, num_samples: int = 1000, seq_len: int = 24) -> Tuple[torch.Tensor, torch.Tensor]:
        """Variable length: Copy last token before STOP token"""
        inputs = []
        targets = []
        STOP_TOKEN = min(49, self.vocab_size - 1)
        
        for _ in range(num_samples):
            # Random sequence length
            actual_len = np.random.randint(5, seq_len - 3)
            
            # Create sequence ending with STOP
            sequence = torch.randint(5, min(30, self.vocab_size), (actual_len,))
            last_token = sequence[-1]
            
            # Input: [sequence] [STOP] [QUERY] [padding]
            input_seq = torch.cat([
                sequence,
                torch.tensor([STOP_TOKEN, self.QUERY_TOKEN]),
                torch.zeros(seq_len - actual_len - 2, dtype=torch.long)
            ])
            
            # Target: predict last token before STOP
            target_seq = torch.zeros(seq_len, dtype=torch.long)
            target_seq[actual_len + 1] = last_token  # Position after QUERY
    ...
    def run_all_tests(self, num_splats: int = 12) -> List[TestResult]:
        ...
        # Memory-conscious tasks - ordered by difficulty
        tasks = [
            ...
            ("Content Match", self.create_content_matching_task, 16, 3000), # Content-based attention
            ("Inhibition", self.create_inhibition_task, 16, 3000),         # Selective attention
            ("Induction", self.create_induction_task, 20, 4000),           # Pattern completion (hardest)
        ]
        
        # Add harder tests only if we have enough memory
        if self.device.type == 'cuda':
            gpu_memory = torch.cuda.get_device_properties(0).total_memory / 1024**3
            if gpu_memory >= 6:
                tasks.extend([
                    ("Multi-step", self.create_multistep_task, 20, 4000),      # Multi-step reasoning
                    ("Interference", self.create_interference_task, 18, 3500),  # Competing patterns
                    ("Variable Length", self.create_variable_length_task, 24, 4000), # Generalization
                ])
    ...
    def print_summary(self, results: List[TestResult]):
        ...
        # Identify specific patterns in results
        ...
        if weak_tasks:
            ...
            # Analyze what types of tasks SplatFlow struggles with
            complex_weak = [r for r in weak_tasks if r.task_name in ["Induction", "Multi-step", "Interference", "Variable Length"]]
            if complex_weak:
                print(f"   🧠 Specifically struggles with complex reasoning tasks")
        
        if strong_tasks:
            print(f"\n🎯 SplatFlow excels at: {', '.join(r.task_name for r in strong_tasks)}")
            
            # Analyze what types of tasks SplatFlow excels at
            basic_strong = [r for r in strong_tasks if r.task_name in ["Simple Copy", "Next Token", "Copying"]]
            reasoning_strong = [r for r in strong_tasks if r.task_name in ["Position", "Content Match", "Inhibition"]]
            advanced_strong = [r for r in strong_tasks if r.task_name in ["Induction", "Multi-step", "Interference"]]
            
            if len(basic_strong) == len(strong_tasks):
                print(f"   📝 Advantage primarily in memorization/copying tasks")
            elif reasoning_strong:
                print(f"   🧠 Shows advantages in reasoning tasks too")
            if advanced_strong:
                print(f"   🚀 Handles even complex multi-step reasoning better")
...
def main():
    ...
    print(f"\n🧪 Test Suite Overview:")
    print(f"   📝 Basic Tasks: Simple copy, next token prediction, multi-token copying")
    print(f"   🧠 Reasoning Tasks: Position-based attention, content matching, selective attention")
    print(f"   🚀 Advanced Tasks: Induction heads, multi-step reasoning, interference resistance")
    print(f"   🎯 Goal: Test where SplatFlow's O(n*k) efficiency helps vs hurts capability")
    ...
    try:
        ...
        basic_tasks = [r for r in results if r.task_name in ["Simple Copy", "Next Token", "Copying"]]
        reasoning_tasks = [r for r in results if r.task_name in ["Position", "Content Match", "Inhibition"]]
        advanced_tasks = [r for r in results if r.task_name in ["Induction", "Multi-step", "Interference", "Variable Length"]]
        ...
        if basic_tasks:
            basic_splat_avg = np.mean([r.splatflow_accuracy for r in basic_tasks])
            basic_std_avg = np.mean([r.standard_accuracy for r in basic_tasks])
            print(f"      Basic Tasks (memorization): SplatFlow {basic_splat_avg:.3f} vs Standard {basic_std_avg:.3f}")
        
        if reasoning_tasks:
            reasoning_splat_avg = np.mean([r.splatflow_accuracy for r in reasoning_tasks])
            reasoning_std_avg = np.mean([r.standard_accuracy for r in reasoning_tasks])
            print(f"      Reasoning Tasks: SplatFlow {reasoning_splat_avg:.3f} vs Standard {reasoning_std_avg:.3f}")
        
        if advanced_tasks:
            advanced_splat_avg = np.mean([r.splatflow_accuracy for r in advanced_tasks])
            advanced_std_avg = np.mean([r.standard_accuracy for r in advanced_tasks])
            print(f"      Advanced Tasks: SplatFlow {advanced_splat_avg:.3f} vs Standard {advanced_std_avg:.3f}")
        ...
        if len(learnable_tasks) == 0:
            ...
            splatflow_better = [r for r in learnable_tasks if r.splatflow_accuracy > r.standard_accuracy + 0.05]
            standard_better = [r for r in learnable_tasks if r.standard_accuracy > r.splatflow_accuracy + 0.05]
        ...
        induction_results = [r for r in results if r.task_name == "Induction"]
        if induction_results:
            induction = induction_results[0]
            print(f"   🧠 CRITICAL TEST - Induction Heads:")
            print(f"      SplatFlow: {induction.splatflow_accuracy:.3f}, Standard: {induction.standard_accuracy:.3f}")
            if induction.splatflow_accuracy > 0.8:
                print(f"      ✅ SplatFlow handles complex reasoning!")
            elif induction.standard_accuracy > induction.splatflow_accuracy + 0.1:
                print(f"      ❌ SplatFlow fails at complex reasoning - expressiveness limitation confirmed")
            else:
                print(f"      ⚠️  Both models struggle with complex reasoning")
        
        # Look for capability ceiling
        if len(results) > 4:
            simple_avg = np.mean([r.splatflow_accuracy for r in results[:3]])  # First 3 tasks
            complex_avg = np.mean([r.splatflow_accuracy for r in results[3:]])  # Rest
            print(f"   📈 Capability Scaling:")
            print(f"      Simple tasks: {simple_avg:.3f}")
            print(f"      Complex tasks: {complex_avg:.3f}")
            if simple_avg - complex_avg > 0.3:
                print(f"      ⚠️  SplatFlow shows capability ceiling - performance drops on harder tasks")
        ...
        print(f"\n💡 INTERPRETATION:")
        print(f"   📊 These results reveal SplatFlow's fundamental reasoning capabilities")
        print(f"   🔍 Performance pattern shows where information bottleneck helps vs hurts")
        print(f"   ⚡ Consistent speed advantages suggest O(n*k) efficiency claims are valid")
        
        if len(results) >= 5:
            print(f"\n🔬 SCIENTIFIC CONCLUSIONS:")
            
            # Calculate overall advantage
            avg_splat = np.mean([r.splatflow_accuracy for r in results])
            avg_std = np.mean([r.standard_accuracy for r in results])
            advantage = avg_splat - avg_std
            
            print(f"   📈 Overall Performance: SplatFlow {avg_splat:.3f} vs Standard {avg_std:.3f}")
            print(f"   📊 Average Advantage: {advantage:+.3f}")
            
            if advantage > 0.5:
                print(f"   🏆 BREAKTHROUGH: SplatFlow shows massive superiority across fundamental tasks")
                print(f"   ✅ Information bottleneck helps rather than hurts basic reasoning")
            elif advantage > 0.2:
                print(f"   🎯 SIGNIFICANT: SplatFlow consistently outperforms on pattern learning")
                print(f"   ✅ Speed advantages come with capability improvements")
            elif advantage > 0.0:
                print(f"   📊 MODEST: SplatFlow slightly better while being much faster")
                print(f"   ✅ No evidence of expressiveness limitations")
            else:
                print(f"   ⚠️  LIMITATION: Standard attention outperforms despite speed disadvantage")
                print(f"   ❌ Information bottleneck may hurt complex reasoning")
            
            # Specific insights based on task types
            reasoning_results = [r for r in results if r.task_name in ["Induction", "Multi-step", "Interference"]]
            if reasoning_results:
                reasoning_avg = np.mean([r.splatflow_accuracy for r in reasoning_results])
                print(f"   🧠 Complex Reasoning Average: {reasoning_avg:.3f}")
                if reasoning_avg > 0.8:
                    print(f"       ✅ SplatFlow handles multi-step reasoning well")
                elif reasoning_avg > 0.5:
                    print(f"       ⚠️  SplatFlow struggles with but can learn complex reasoning")
                else:
                    print(f"       ❌ SplatFlow fails complex reasoning - expressiveness ceiling confirmed")
