"""
Fixed Enhanced Biological SplatFlow Training with Extended Evolution

Fixed the method signature issue in _apply_enhanced_biological_adaptation call.
"""
...
class EnhancedBiologicalSplatAttentionLayer(nn.Module):
    ...
    def forward(self, token_embeddings: torch.Tensor, 
        ...
        if self.training and self.adaptation_enabled and self.forward_count % self.adaptation_frequency == 0:
            with torch.no_grad():
                # Create layer-specific loss signal if not provided
                if loss_per_token is None:
                    # Use layer-specific adaptation signal based on output characteristics
                    output_variance = torch.var(output, dim=-1)
                    output_magnitude = torch.norm(output, dim=-1)
                    layer_loss = output_variance + output_magnitude * 0.1
                else:
                    layer_loss = loss_per_token
                
                # FIXED: Apply adaptation with correct signature (removed extra 'output' argument)
                self._apply_enhanced_biological_adaptation(affinities, layer_loss)
    ...
    def debug_layer_adaptation_activity(self):
        """Debug method to show adaptation activity across layers"""
        print(f"\n🔍 Layer Adaptation Debug:")
        for i, layer in enumerate(self.layers):
            attention_layer = layer.attention
            if hasattr(attention_layer, '_last_adaptation_debug'):
                debug_info = attention_layer._last_adaptation_debug
                print(f"   Layer {i}: useful={debug_info['useful_splats']}, "
                      f"ready={debug_info['ready_for_mitosis']}, "
                      f"changes={debug_info['usefulness_changes']}, "
                      f"divide={debug_info['splats_to_divide']}, "
                      f"signal={debug_info['adaptation_signal_strength']:.4f}")
            else:
                print(f"   Layer {i}: No adaptation debug info available")
        ...
        for i, layer in enumerate(self.layers):
            ...
            if hasattr(attention_layer, '_last_adaptation_debug'):
                debug_info = attention_layer._last_adaptation_debug
                if debug_info['useful_splats'] > 0 or debug_info['usefulness_changes'] > 0:
                    active_layers += 1
        
        if active_layers == 0:
            print(f"   🚨 WARNING: No layers showing adaptation activity!")
            print(f"   💡 This suggests adaptation signals aren't reaching the layers properly")
        elif active_layers < len(self.layers) // 2:
            print(f"   ⚠️  Only {active_layers}/{len(self.layers)} layers are active")
        else:
            print(f"   ✅ {active_layers}/{len(self.layers)} layers showing adaptation activity")
...
class EnhancedBiologicalSplatTransformerLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attention_mask: Optional[torch.Tensor] = None,
                loss_per_token: Optional[torch.Tensor] = None) -> torch.Tensor:
        """Forward pass with enhanced layer-specific biological adaptation"""
        
        # Self-attention with residual connection and layer-specific adaptation
        ...
        # Create layer-specific loss signal for next layer based on how much this layer changed the input
        # This gives each layer its own adaptation signal
        if loss_per_token is not None:
            # Measure how much this layer transformed the input
            layer_transformation = torch.norm(final_output - x, dim=-1)  # [batch, seq_len]
            # Combine with input loss signal
            updated_loss = loss_per_token + layer_transformation * 0.1
        else:
            # Create layer-specific signal based on output characteristics
            output_variance = torch.var(final_output, dim=-1)
            output_magnitude = torch.norm(final_output, dim=-1) 
            updated_loss = output_variance + output_magnitude * 0.05
        
        # Store the updated loss signal for debugging (could be used by model)
...
class EnhancedBiologicalSplatFlowGPT(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor, 
        ...
        """Forward pass through the enhanced biological splat-flow model with layer-specific adaptation"""
        ...
        # Initial layer-specific loss signal for adaptation
        if compute_loss_per_token and self.training:
            # Enhanced initial loss signal based on input embedding characteristics
            embedding_variance = torch.var(x, dim=-1)
            embedding_norm = torch.norm(x, dim=-1)
            token_complexity = torch.sum(torch.abs(x), dim=-1)  # L1 norm as complexity measure
            
            # Combined initial signal
            current_loss_signal = embedding_variance + embedding_norm * 0.1 + token_complexity * 0.05
        else:
            current_loss_signal = None
        
        # Process through enhanced biological splat-flow layers with layer-specific signals
        for layer_idx, layer in enumerate(self.layers):
            # Each layer gets its own version of the loss signal
            if current_loss_signal is not None:
                # Add layer-specific variation to prevent identical signals
                layer_variation = torch.randn_like(current_loss_signal) * 0.02 * (layer_idx + 1)
                layer_specific_loss = current_loss_signal + layer_variation
            else:
                layer_specific_loss = None
            
            # Forward through layer with layer-specific adaptation signal
            x = layer(x, attention_mask, layer_specific_loss)
            
            # Update loss signal for next layer based on how this layer changed the representation
            if current_loss_signal is not None and hasattr(layer, '_last_layer_loss'):
                current_loss_signal = layer._last_layer_loss
            elif current_loss_signal is not None:
                # Fallback: update based on layer output
                layer_transformation = torch.var(x, dim=-1)
                current_loss_signal = current_loss_signal * 0.8 + layer_transformation * 0.2
    ...
    def debug_layer_adaptation_activity(self):
        ...
        for i, layer in enumerate(self.layers):
            attention_layer = layer.attention
            stats = attention_layer.get_enhanced_adaptation_stats()
            
            num_splats = stats.get('num_splats', 0)
            usefulness = stats.get('avg_usefulness', 0)
            max_usefulness = stats.get('max_usefulness', 0)
            generation = stats.get('max_generation', 0)
            births = stats.get('birth_count', 0)
            deaths = stats.get('death_count', 0)
            ready = stats.get('ready_for_mitosis', 0)
            exploration = stats.get('exploration_activity', 0)
            
            # Activity indicator
            if births > 0 or deaths > 0:
                activity_emoji = "🔥"  # Active evolution
            elif ready > 0:
                activity_emoji = "⚡"  # Ready for evolution
            elif usefulness > 1.01:
                activity_emoji = "📈"  # Learning
            else:
                activity_emoji = "😴"  # Dormant
            
            print(f"   {activity_emoji} Layer {i}: {num_splats} splats, "
                  f"usefulness={usefulness:.3f} (max={max_usefulness:.3f}), "
                  f"gen={generation}, births={births}, deaths={deaths}, "
                  f"ready={ready}, explore={exploration:.3f}")
        ...
        for i, layer in enumerate(self.layers):
            attention_layer = layer.attention
            stats = attention_layer.get_enhanced_adaptation_stats()
            if stats.get('birth_count', 0) > 0 or stats.get('avg_usefulness', 0) > 1.01:
                active_layers += 1
...
def report_enhanced_adaptation_stats(model, epoch: int):
    """Report enhanced biological adaptation statistics with layer-specific details"""
    ...
    # Show all layers with detailed biological metrics
    print(f"   Layer-specific biological activity:")
    for i in range(len([k for k in stats.keys() if k.startswith('layer_')])):
        layer_stats = stats.get(f'layer_{i}', {})
        num_splats = layer_stats.get('num_splats', 0)
        usefulness = layer_stats.get('avg_usefulness', 0)
        max_usefulness = layer_stats.get('max_usefulness', 0)
        generation = layer_stats.get('max_generation', 0)
        births = layer_stats.get('birth_count', 0)
        deaths = layer_stats.get('death_count', 0)
        ready = layer_stats.get('ready_for_mitosis', 0)
        exploration = layer_stats.get('exploration_activity', 0)
    ...
    active_layers = sum(1 for i in range(4) if stats.get(f'layer_{i}', {}).get('birth_count', 0) > 0 or 
                                                 stats.get(f'layer_{i}', {}).get('avg_usefulness', 0) > 1.01)
    
    if active_layers >= 3:
        print(f"   🎉 HEALTHY ECOSYSTEM: {active_layers}/4 layers show biological activity!")
    elif active_layers >= 2:
        print(f"   📊 MODERATE ECOSYSTEM: {active_layers}/4 layers show biological activity")
    elif active_layers >= 1:
        print(f"   ⚠️  LIMITED ECOSYSTEM: Only {active_layers}/4 layers show biological activity")
    else:
        print(f"   🚨 DORMANT ECOSYSTEM: No layers showing biological activity - check adaptation signals!")
        
    # Give specific recommendations if layers are dormant
    if active_layers < 2:
        print(f"   💡 Suggestion: Layer-specific adaptation signals may need tuning")
        dormant_layers = [i for i in range(4) if stats.get(f'layer_{i}', {}).get('avg_usefulness', 0) <= 1.01]
        if dormant_layers:
            print(f"      Dormant layers: {dormant_layers}")
            print(f"      These layers may not be receiving proper adaptation signals")
...
def train_enhanced_biological_splatflow():
    ...
    def lr_lambda(epoch):
        if epoch < warmup_steps:
            return (epoch + 1) / warmup_steps  # Start from learning_rate/warmup_steps, not 0
    ...
    for epoch in range(config['epochs']):
        ...
        # Debug layer-specific adaptation activity every 5 epochs
        if (epoch + 1) % 5 == 0:
            model.debug_layer_adaptation_activity()
