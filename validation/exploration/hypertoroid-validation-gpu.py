...
# CUDA setup with memory optimization
...
if torch.cuda.is_available():
    ...
    if total_memory < 6:  # Less than 6GB
        print(f"Detected {total_memory}GB VRAM - using scaled settings for better GPU utilization")
...
def create_chain_connection_task(seq_len: int = 32, batch_size: int = 8) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    """
    # Balanced settings for 5GB GPU - sweet spot between performance and memory
    if MEMORY_OPTIMIZED:
        embedding_dim = 64   # Sweet spot: 2x original, 1/2 previous
        batch_size = min(batch_size * 2, 16)  # Moderate batches: up to 16
        seq_len = min(seq_len + 8, 40)  # Moderate sequences: up to 40
    else:
        embedding_dim = 64
    
    # Create sequences with chain structure
    sequences = torch.randn(batch_size, seq_len, embedding_dim, device=device)
    ...
    for batch_idx in range(batch_size):
        ...
        chain_repr = torch.randn(embedding_dim, device=device)
...
def create_torus_intersection_task(seq_len: int = 28, batch_size: int = 6) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    if MEMORY_OPTIMIZED:
        embedding_dim = 64   # Sweet spot: 2x original, 1/2 previous
        batch_size = min(batch_size * 2, 12)  # Moderate batches: up to 12
        seq_len = min(seq_len + 8, 36)  # Moderate sequences: up to 36
    else:
        embedding_dim = 64
    ...
    for batch_idx in range(batch_size):
        ...
        for pos1, pos2 in opposite_pairs:
            if pos1 < seq_len and pos2 < seq_len:
                ...
                shared_component = torch.randn(embedding_dim, device=device) * 0.3
...
def train_and_compare_evolution(task_name: str,
    ...
    # Balanced settings for 5GB GPU - sweet spot between performance and memory
    if MEMORY_OPTIMIZED:
        n_splats = 8    # Moderate: between 4 and 16
        n_heads = 4     # Moderate: between 2 and 8  
        print(f"Balanced GPU settings: {n_splats} splats, {n_heads} heads")
    else:
        n_splats = 8
        n_heads = 4
    
    # Create models
    torus_model = EvolvingTorusAttention(embedding_dim, n_splats=n_splats, n_heads=n_heads).to(device)
    standard_model = StandardSplatAttention(embedding_dim, n_splats=n_splats, n_heads=n_heads).to(device)
    
    # Move data to device
    x_train, y_train = x_train.to(device), y_train.to(device)
    x_test, y_test = x_test.to(device), y_test.to(device)
    
    # Optimizers with settings for better GPU utilization
    if MEMORY_OPTIMIZED:
        # Standard learning rate, no gradient accumulation needed with more memory
    ...
    for epoch in range(n_epochs):
        ...
        if epoch % 25 == 0:
            ...
            if torch.cuda.is_available():
                ...
                memory_percent = (memory_used / (total_memory * 1024)) * 100 if total_memory > 0 else 0
                #print(f"  GPU Memory Used: {memory_used} MB ({memory_percent:.1f}% of {total_memory}GB)")
...
def test_specific_torus_capabilities():
    ...
    test_input = torch.randn(4, seq_len, embedding_dim, device=device)
    
    # Make first and last tokens very similar (they should wrap around torus)
    shared_pattern = torch.randn(embedding_dim, device=device)
    test_input[:, 0, :] = shared_pattern + torch.randn(4, embedding_dim, device=device) * 0.1
    test_input[:, -1, :] = shared_pattern + torch.randn(4, embedding_dim, device=device) * 0.1
    
    # Create evolved torus model
    torus_model = EvolvingTorusAttention(embedding_dim, n_splats=6, n_heads=2).to(device)
    ...
    multi_hop_input = torch.randn(2, seq_len, embedding_dim, device=device)
    
    # Create two similar patterns that should connect via entry points
    pattern_a = torch.randn(embedding_dim, device=device)
    pattern_b = torch.randn(embedding_dim, device=device)
