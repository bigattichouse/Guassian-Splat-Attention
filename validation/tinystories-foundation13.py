"""
Multi-Model DirectionalGSA - Phase 1: Real Q/K/V Projections
============================================================

Building on the proven integration foundation, this version implements:
- Real Q/K/V projection extraction (tested and working)
- Basic splat attention computation
- Conservative directional components
...
class UniversalDirectionalGSA(nn.Module):
    """
    Universal DirectionalGSA - Phase 1: Real Q/K/V with Basic Splat Attention
    
    This version builds on the proven integration foundation by implementing
    real Q/K/V projection extraction and basic splat attention computation.
    """
    ...
    def __init__(self, original_attention, config: ModelConfig, layer_id: int):
        ...
        # Core splat parameters - PROVEN device handling
        ...
        logger.info(f"UniversalDirectionalGSA Phase 1 initialized for {config.attention_class_name} layer {layer_id}")
    ...
    def _get_projections(self, hidden_states):
        """Get Q, K, V projections - PROVEN WORKING with GQA support."""
        try:
            ...
            if hasattr(self.original_attention, self.config.q_proj_name):
                ...
                if self.config.q_proj_name == "Wqkv":  # Phi-style combined projection
                    ...
                    # Handle GQA: different number of query vs key/value heads
                    if self.is_gqa:
                        # Query: full number of heads
                        query = query.view(batch_size, seq_len, self.num_heads, self.head_dim).transpose(1, 2)
                        
                        # Key/Value: fewer heads, need to expand
                        ...
                        # Standard MHA: same number of heads for all
                        query = query.view(batch_size, seq_len, self.num_heads, self.head_dim).transpose(1, 2)
    ...
    def forward(self, hidden_states, attention_mask=None, **kwargs):
        ...
        try:
            ...
            # Very conservative activation threshold
            if gsa_strength < 0.005:
                return self.original_attention(hidden_states, attention_mask=attention_mask, **kwargs)
            ...
            try:
                directional_hidden_states = self._compute_directional_gsa(hidden_states, attention_mask)
                
                # Very conservative blending - only 1% max influence
                blend_factor = min(0.01, gsa_strength * 0.05)
            ...
            except Exception as gsa_error:
                logger.warning(f"DirectionalGSA computation failed, using standard attention: {gsa_error}")
    ...
    def _compute_directional_gsa(self, hidden_states, attention_mask=None):
        """Compute DirectionalGSA output - Phase 1: Real Q/K/V with Basic Splats."""
        try:
            # Get real Q, K, V projections - PROVEN WORKING
            query, key, value = self._get_projections(hidden_states)
            
            # Validate tensor shapes
            if query.dim() != 4 or key.dim() != 4 or value.dim() != 4:
                logger.warning("Invalid tensor dimensions from projections")
            ...
            if num_heads != self.num_heads or head_dim != self.head_dim:
                logger.warning(f"Dimension mismatch: expected ({self.num_heads}, {self.head_dim}), got ({num_heads}, {head_dim})")
                return hidden_states
            
            # Get splat parameters with conservative scaling
            ...
            # Normalize and scale parameters conservatively
            positions = F.normalize(positions, dim=-1) * math.sqrt(head_dim) * 0.2  # Reduced scaling
            directions = F.normalize(directions, dim=-1)
            scales = torch.exp(log_scales).clamp(min=0.5, max=1.0)  # Conservative range
            amplitudes = F.softmax(log_amplitudes, dim=-1)
            
            # Directional strength - start conservative
            dir_strength = torch.sigmoid(self.directional_strength).to(device) * 0.5  # Max 50%
            
            # Compute splat-based attention
            attention_weights = self._compute_splat_attention(
    ...
    def _compute_splat_attention(self, query, key, positions, directions, scales, amplitudes, dir_strength):
        """Compute attention weights through splats."""
        batch_size, num_heads, seq_len, head_dim = query.shape
        device = query.device
        ...
        # Compute attention for each splat
        for s in range(min(self.n_splats, positions.shape[1])):
            try:
                # Get splat parameters for this specific splat
                pos_s = positions[:, s, :]  # [H, D]
                dir_s = directions[:, s, :]  # [H, D]
                scale_s = scales[:, s]  # [H]
                amp_s = amplitudes[:, s]  # [H]
                
                # Compute affinities for query and key separately
                query_affinity = self._compute_token_splat_affinity(
                    query, pos_s, dir_s, scale_s, dir_strength
                )  # [B, H, T]
                
                key_affinity = self._compute_token_splat_affinity(
                    key, pos_s, dir_s, scale_s, dir_strength
                )  # [B, H, T]
                
                # Compute attention through this splat: outer product of affinities
                splat_attention = torch.einsum('bht,bhs->bhts', query_affinity, key_affinity)
                
                # Scale by splat amplitude
                splat_attention = splat_attention * amp_s.view(1, -1, 1, 1)
                
                # Add to total attention
                attention_weights = attention_weights + splat_attention
                
            except Exception as e:
                logger.warning(f"Error computing splat {s}: {e}")
                continue
        
        return attention_weights
    
    def _compute_token_splat_affinity(self, tokens, position, direction, scale, dir_strength):
        """Compute affinity between tokens and a single splat."""
        batch_size, num_heads, seq_len, head_dim = tokens.shape
        
        # Expand position and direction for broadcasting
        ...
        # Spatial affinity (Gaussian distance from splat center)
        ...
        # Directional affinity (alignment with splat direction)
    ...
    def enable_directional_gsa(self, strength: float = 0.005):
        """Enable DirectionalGSA with Phase 1 implementation."""
        self.enable_gsa = True
        self.set_gsa_strength(strength)
        logger.info(f"Phase 1 DirectionalGSA enabled with strength {strength:.3f}")
...
class MultiModelExperiment:
    ...
    def install_directional_gsa(self) -> bool:
        ...
        logger.info(f"Installing Phase 1 DirectionalGSA on layer {layer_idx}")
        ...
        try:
            ...
            logger.info("✓ Phase 1 DirectionalGSA installed successfully")
            return True
    ...
    def run_gsa_test(self, strength: float = 0.005) -> Dict:
        """Run DirectionalGSA test."""
        logger.info(f"Running Phase 1 DirectionalGSA test with strength {strength:.1%}...")
        
        # Enable GSA with very conservative strength
        ...
        for length in test_lengths:
            logger.info(f"Testing Phase 1 GSA at {length} tokens...")
        ...
        logger.info(f"Phase 1 GSA success rate: {overall_success:.1%}")
    ...
    def run_complete_test(self) -> Dict:
        ...
        logger.info("="*80)
        logger.info(f"🧭 MULTI-MODEL DIRECTIONAL GSA TEST - {self.model_type.upper()} (PHASE 1)")
        logger.info("="*80)
        ...
        try:
            ...
            # Phase 4: GSA test
            gsa_results = self.run_gsa_test(strength=0.005)  # 0.5% strength
            ...
            if gsa_success > baseline_success + 0.1:
                verdict = "IMPROVEMENT"
                detail = f"Phase 1 DirectionalGSA improved performance: {baseline_success:.1%} → {gsa_success:.1%}"
            elif abs(gsa_success - baseline_success) <= 0.1:
                verdict = "STABLE"
                detail = f"Phase 1 DirectionalGSA maintained performance: {baseline_success:.1%} ≈ {gsa_success:.1%}"
            else:
                verdict = "DEGRADATION"
                detail = f"Phase 1 DirectionalGSA degraded performance: {baseline_success:.1%} → {gsa_success:.1%}"
            ...
            # Save results
            results_file = f"multi_model_gsa_results_{self.model_type}_phase1.json"
            ...
            logger.info(f"Phase 1 DirectionalGSA: {gsa_success:.1%}")
...
def main():
    ...
    # Test multiple models with Phase 1 implementation
    models_to_test = ["qwen2", "tinyllama"]
    
    overall_results = {}
    
    for model_type in models_to_test:
        logger.info(f"\n🧭 Testing {model_type.upper()} (PHASE 1 REAL GSA)")
    ...
    # Final summary
    logger.info("\n" + "="*80)
    logger.info("🏁 MULTI-MODEL EXPERIMENT COMPLETE (PHASE 1 REAL GSA)")
    logger.info("="*80)
