...
class SimpleTransformer(nn.Module):
    """Multi-layer transformer for testing attention patterns"""
    ...
    def __init__(self, vocab_size: int, model_dim: int, attention_layer: nn.Module, 
        ...
        # Multiple layers for better learning capacity
        ...
        for i in range(num_layers):
            if i == 0:
                ...
                if hasattr(attention_layer, 'num_splats'):
                    # SplatFlow layer
                    attn_layer = TrueSplatAttentionLayer(model_dim, attention_layer.num_splats)
                else:
                    # Standard attention layer
                    attn_layer = SimpleStandardAttention(model_dim)
...
class AttentionPatternTester:
    ...
    def create_next_token_task(self, num_samples: int = 1000, seq_len: int = 8) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for _ in range(num_samples):
            ...
            start_token = torch.randint(5, min(15, self.vocab_size - 3), (1,))
    ...
    def train_and_evaluate(self, model: nn.Module, inputs: torch.Tensor, targets: torch.Tensor, 
        ...
        """Train model on task and return performance metrics with improved training"""
                ...
                with torch.no_grad():
                    ...
                    if test_mask.sum() > 0:
                        ...
                        if accuracy > best_accuracy:
                            ...
                            # Check convergence (90% accuracy for easier threshold)
                            if accuracy > 0.90 and convergence_step == max_steps:
    ...
    def run_all_tests(self, num_splats: int = 16) -> List[TestResult]:
        """Run all attention pattern tests from simple to complex"""
        print(f"\n🧪 Running Attention Pattern Tests (Progressive Difficulty)")
        print(f"=" * 60)
        
        # Progressive tasks from simple to complex
        tasks = [
            ("Simple Copy", self.create_simple_copy_task, 8, 3000),    # Single token copy
            ("Next Token", self.create_next_token_task, 8, 3000),     # Sequence prediction
            ("Copying", self.create_copying_task, 12, 4000),          # Multi-token copy
            ("Position", self.create_position_task, 16, 5000),        # Position-based
            ("Induction", self.create_induction_task, 20, 6000)       # Pattern completion
        ]
        ...
        for task_name, task_creator, seq_len, max_steps in tasks:
            ...
            if task_name in ["Simple Copy", "Next Token"]:
                inputs, targets = task_creator(num_samples=800, seq_len=seq_len)
            else:
                inputs, targets = task_creator(num_samples=600, seq_len=seq_len)
            ...
            splatflow_model = SimpleTransformer(self.vocab_size, self.model_dim, splatflow_attn, 
                                              max_seq_len=seq_len + 5, num_layers=3)
            ...
            standard_attn = SimpleStandardAttention(self.model_dim, num_heads=8)
            ...
            # Early termination if both models fail simple tasks
            if task_name in ["Simple Copy", "Next Token"] and max(splatflow_results['accuracy'], 
                                                                 standard_results['accuracy']) < 0.5:
                print(f"   ⚠️  Both models failing on {task_name} - may need larger models or more training")
...
def main():
    """Run the comprehensive pattern testing suite"""
    print("🚀 SplatFlow Fundamental Pattern Tests v2.0")
    print("Testing the building blocks of attention with proper model sizes...")
    
    # Initialize tester with larger, more capable models
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    model_dim = 256 if torch.cuda.is_available() else 128  # Adjust based on GPU memory
    ...
    print(f"   Progressive task difficulty")
    print(f"   Proper train/test splits")
    print(f"   Early stopping and LR scheduling")
    
    # Run tests
    try:
        results = tester.run_all_tests(num_splats=16)
        tester.print_summary(results)
        
        print(f"\n🎯 COMPREHENSIVE ANALYSIS:")
        
        # Check if models actually learned the tasks
        learnable_tasks = [r for r in results if max(r.splatflow_accuracy, r.standard_accuracy) > 0.8]
        failed_tasks = [r for r in results if max(r.splatflow_accuracy, r.standard_accuracy) < 0.5]
        
        if len(learnable_tasks) == 0:
            print(f"   ❌ CRITICAL: Both models failed all tasks - need larger models or more training")
        elif len(failed_tasks) > len(results) // 2:
            print(f"   ⚠️  WARNING: Most tasks too difficult for current model size")
        else:
            print(f"   ✅ SUCCESS: Models successfully learned {len(learnable_tasks)}/{len(results)} tasks")
            
            # Analyze SplatFlow performance on learned tasks
            splatflow_better = [r for r in learnable_tasks if r.splatflow_accuracy > r.standard_accuracy + 0.02]
            standard_better = [r for r in learnable_tasks if r.standard_accuracy > r.splatflow_accuracy + 0.02]
        ...
        # Specific insights
        simple_tasks = [r for r in results if r.task_name in ["Simple Copy", "Next Token"]]
        complex_tasks = [r for r in results if r.task_name in ["Induction", "Position"]]
        
        if simple_tasks:
            simple_splat_avg = np.mean([r.splatflow_accuracy for r in simple_tasks])
            simple_std_avg = np.mean([r.standard_accuracy for r in simple_tasks])
            print(f"   📊 Simple tasks: SplatFlow {simple_splat_avg:.3f} vs Standard {simple_std_avg:.3f}")
        
        if complex_tasks:
            complex_splat_avg = np.mean([r.splatflow_accuracy for r in complex_tasks])
            complex_std_avg = np.mean([r.standard_accuracy for r in complex_tasks])
            print(f"   📊 Complex tasks: SplatFlow {complex_splat_avg:.3f} vs Standard {complex_std_avg:.3f}")
        
        print(f"\n💡 INTERPRETATION:")
        print(f"   These results show SplatFlow's ability to learn fundamental reasoning patterns")
        print(f"   High performance suggests the information bottleneck isn't catastrophic")
        print(f"   Speed advantages become more credible with preserved learning capability")
