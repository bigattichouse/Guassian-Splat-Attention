...
def run_aggressive_scaling_test():
    ...
    for i, config in enumerate(test_configs):
        ...
        try:
            ...
            # Run both inference and training steps to ensure full system works
            times = []
            training_times = []
            peak_memory = 0
            
            print(f"   ⏱️  Running inference tests...")
            
            # First: Test inference (faster, lower memory)
            for batch_idx, batch in enumerate(dataloader):
                ...
                try:
                    ...
                    print(f"     Inference {batch_idx + 1}: {step_time:.3f}s")
                    
                except Exception as e:
                    print(f"     ❌ Inference {batch_idx + 1} failed: {e}")
                    break
            
            # Second: Test actual training steps to ensure splat adaptation works
            print(f"   🏋️  Running training steps...")
            model.train()
            optimizer = trainer.optimizer
            
            for batch_idx, batch in enumerate(dataloader):
                if batch_idx >= 2:  # Just 2 training steps (higher memory)
                    break
                ...
                try:
                    ...
                    optimizer.zero_grad()
                    logits = model(input_ids)  # FIXED: Remove epoch parameter
                    
                    loss = torch.nn.functional.cross_entropy(
                        logits.reshape(-1, logits.size(-1)),
                        targets.reshape(-1),
                        ignore_index=trainer.tokenizer.pad_token_id
                    )
                    ...
                    # Track peak memory (training uses more memory)
                    current_memory = get_gpu_memory_info()
                    if current_memory:
                        peak_memory = max(peak_memory, current_memory['allocated'])
                    
                    print(f"     Training {batch_idx + 1}: {training_time:.3f}s, loss={loss.item():.4f}")
                    
                    # Get quick splat health check
                    try:
                        from splatflow import get_quick_model_stats
                        stats = get_quick_model_stats(model)
                        print(f"       Splats: {stats['healthy_splats']}/{stats['total_splats']} healthy, {stats['total_births']} births")
                    except Exception:
                        pass
                    
                except Exception as e:
                    print(f"     ❌ Training {batch_idx + 1} failed: {e}")
                    break
            ...
            if times and training_times:
                avg_inference_time = sum(times) / len(times)
                avg_training_time = sum(training_times) / len(training_times)
                tokens_per_second_inf = (seq_len * config['batch_size']) / avg_inference_time
                tokens_per_second_train = (seq_len * config['batch_size']) / avg_training_time
                ...
                result = {
                    ...
                    'tokens_per_second_inf': tokens_per_second_inf,
                ...
                print(f"   ✅ SUCCESS: Inf={avg_inference_time:.3f}s, Train={avg_training_time:.3f}s, {actual_memory_gb:.3f}GB peak, {memory_advantage:.1f}x advantage")
    ...
    print("=" * 110)
    print(f"{'Context':<8} {'Inference':<10} {'Training':<10} {'Tok/s Inf':<10} {'Tok/s Tr':<10} {'Actual GB':<10} {'Std GB':<8} {'Advantage':<10}")
    print("-" * 110)
    
    for r in results:
        print(f"{r['seq_length']:<8} "
              f"{r['avg_inference_time']:.3f}s{'':<3} "
              f"{r['avg_training_time']:.3f}s{'':<3} "
              f"{r['tokens_per_second_inf']:.0f}{'':<6} "
              f"{r['tokens_per_second_train']:.0f}{'':<6} "
    ...
    if len(results) >= 2:
        ...
        inference_speeds = [r['tokens_per_second_inf'] for r in results]
        training_speeds = [r['tokens_per_second_train'] for r in results]
        ...
        print(f"   ⏱️  Performance: {results[-1]['tokens_per_second_inf']:.0f} tok/s inference, {results[-1]['tokens_per_second_train']:.0f} tok/s training")
        ...
        print(f"   Performance at max context: {max_result['tokens_per_second_inf']:.0f} tok/s (inference), {max_result['tokens_per_second_train']:.0f} tok/s (training)")
        
        if max_context >= 8192:
            print(f"   🏆 BREAKTHROUGH: Achieved {max_context:,} token context with FULL TRAINING - impossible with standard attention!")
        
        # Calculate training overhead vs inference
        if 'avg_training_time' in max_result and 'avg_inference_time' in max_result:
            training_overhead = max_result['avg_training_time'] / max_result['avg_inference_time']
            print(f"   🏋️  Training overhead: {training_overhead:.1f}x slower than inference (includes gradients + splat adaptation)")
